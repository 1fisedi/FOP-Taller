package tiendavirtual.tienda_minima_v11.org.pl.eshopping.dto;

import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.sql.Statement;
import tiendavirtual.tienda_minima_v11.org.pl.eshopping.bd.Conexion;
...
public class CategoriaDAO {
	...
	public void modificar(Categoria c) {
		try {
			conn = Conexion.getConexion();
			String query = "UPDATE categoriaset nombre=?, descripcion=? WHERE id=?";
			ps = conn.prepareStatement(query);
			ps.setString(1, c.getNombre());
			ps.setString(2, c.getDescripcion());
	...
	public void eliminar(Categoria c) {
		try {
			conn = Conexion.getConexion();
			String query = "DELETE FROM categoriaWHERE id=?";
			ps = conn.prepareStatement(query);
